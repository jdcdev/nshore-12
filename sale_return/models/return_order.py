...
class ReturnOrder(models.Model):
    ...
    def process_return(self):
        ...
        if self.type_partner == 'customer':
            ...
            if line_ids:
                self.process_return_to_stock(line=line_ids)
            ...
            if line_ids_wo_po:
                self.process_return_product_vendor_type(lines=line_ids_wo_po)
                self.product_refund_from_vendor_type(lines=line_ids_wo_po)
    ...
    def process_return_product_vendor_with_po(self, lines):
        ...
        refund = invoice_obj.create(refund_vals)
        for line in lines:
            self.env['account.invoice.line'].create({
                ...
                'invoice_line_tax_ids': [(6, 0, line.tax_id.ids)] or False,
    ...
    def process_return_product_vendor_type(self, lines=None):
    ...
    def process_refund_customer(self, line=None):
        ...
        invoice_ids_customer = False
        # for record in line_record_stock:
        for l in line:
            invoice_ids_customer = l.sale_order_id.invoice_ids.filtered(
        ...
        for order_line in line:
            ...
            self.env['account.invoice.line'].create({
                ...
                'account_id': order_line.product_id.property_account_income_id.id \
                or order_line.product_id.categ_id.property_account_income_categ_id.id \
                or False,
                'quantity': order_line.qty or 0.0,
                'uom_id': order_line.product_id.product_tmpl_id.uom_id.id or False,
                'price_unit': order_line.unit_price or 0.0,
                'invoice_line_tax_ids': [(6, 0, order_line.tax_id.ids)],
...
class ReturnOrderLine(models.Model):
    ...
    # Defined fields
    price_tax = fields.Float(tring='Total Tax', readonly=True, compute='_compute_amount', store=True)
        ...
        for record in self:
            ...
            if record.return_id.type_partner == 'customer':
                ...
                order_line_rec = sales_order_line.filtered(
                    lambda x: x.order_id and x.order_id.partner_id == record.return_id.partner_id)
                sales_order = list(set([line.order_id.id for line in order_line_rec]))
            ...
            # Get value of order line from PO or without PO.
                ...
                purchase_order_line_rec = purchase_order_line.filtered(
                    lambda p: p.order_id and p.order_id.partner_id == record.return_id.supplier_id)
                ...
                if not purchase_order:
                    ...
                    taxes = record.product_id.taxes_id
                    record.tax_id = record.return_id.supplier_id.property_account_position_id.map_tax(taxes, record.product_id, record.return_id.supplier_id)
